...
int main(int argc, char **argv){
    ...
    std::vector<double> launch = {0.815311,-0.967175,-1.75372,0.699112,-0.950685,-0.193282,-0.723655};
        ...
        for (int i=1; i<=num_diffeo;i++){
            ...
            while(ros::Time::now().toSec()-begin < 10 && norm>0.07){
               ...
               if(starting_angles[0]*starting_angles[0]>0.00001){
                    ...
                    for (int i=0; i<7;i++){
                        double vel=resVel(i);
                        desired_velocity[i]=vel;
                        //desired_velocity[i]=actual_velocity[i] + alpha*(0.40*vel-actual_velocity[i]);
                    }
                   ...
                   // while(ros::Time::now().toSec()-tnow<1.0)
                    ...
                    for(int i = 0; i < 7; i++){
                        ...
                        norm=norm+(launch[i]-starting_angles[i])*(launch[i]-starting_angles[i]);
                    }
